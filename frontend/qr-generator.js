/**
 * KnitX Offline QR Code Generator
 * A lightweight, self-contained QR Code generator in pure JavaScript.
 * Renders QR Codes directly onto HTML5 canvases.
 * No external dependencies, perfect for offline industrial Edge-AI deployment.
 * Adopted from public domain QR Code algorithm.
 */

window.KnitXQR = (function () {
    ...
    QRCode.createBytes = function (buffer, rsBlocks) {
        ...
        for (var r = 0; r < rsBlocks.length; r++) {
            ...
            for (var i = 0; i < dcBytes[r].length; i++) {
        ...
        getLengthInBits: function (mode, type) {
            if (1 <= type && type < 10) {
                switch (mode) {
                    ...
                    case QRMode.MODE_KANJI: return 13;
    ...
    QRRSBlock.RS_BLOCK_TABLE = [
        ...
        [2, 121, 97], [2, 48, 22, 2, 49, 23], [4, 40, 18, 2, 41, 19], [4, 40, 14, 4, 41, 15],
    ...
    return {
         ...
         * @param {number} scale Optional module size scale in pixels (default 4)
         */
        render: function (canvas, text, scale) {
            scale = scale || 4;
            var qr = new QRCode(8, QRErrorCorrectionLevel.M); // Type 8 handles up to ~150 chars, perfect for reports
            qr.addData(text);
            qr.make();
